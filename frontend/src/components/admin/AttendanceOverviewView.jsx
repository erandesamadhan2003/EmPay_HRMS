...
export default function AttendanceOverviewView(){
  ...
  const data = useMemo(() => {
    return apiEmps.map((e, idx) => {
      ...
      for (let d = 1; d <= daysInMonth; d++) {
        ...
        if (dow === 0 || dow === 6) {
          rec.push('W');
          hoursArr.push({ day: d, hrs: 0 });
        } else if (userAtt[dateStr]) {
          const st = userAtt[dateStr].status;
          if (st === 'present') { rec.push('P'); }
          else if (st === 'on_leave') { rec.push('L'); }
          else { rec.push('A'); }
          // Calculate hours from check-in/check-out
          let hrs = 0;
          if (userAtt[dateStr].checkIn && userAtt[dateStr].checkOut) {
            hrs = (new Date(userAtt[dateStr].checkOut) - new Date(userAtt[dateStr].checkIn)) / 3600000;
          } else if (st === 'present') { hrs = 8; }
          hoursArr.push({ day: d, hrs: Math.round(hrs * 10) / 10 });
        } else {
          // No record for this day — if date is in future, leave blank; else absent
          if (dt > now) { rec.push('W'); }
          else { rec.push('A'); }
          hoursArr.push({ day: d, hrs: 0 });
        }
      }
      return { id: uid || idx + 1, name: nm, dept: e.profile?.department?.name || '—', initials: nm.split(' ').map(x => x[0]).join('').toUpperCase(), days: rec, hoursData: hoursArr };
    });
  }, [apiEmps, attMap, year, month, daysInMonth]);

  const deptOptions = ['All', ...new Set([...apiDepts, ...data.map(e => e.dept)].filter(d => d && d !== '—'))];
  ...
  const openModal = (emp) => {
    ...
    setModal({ ...emp, present, absent, leave, hours: emp.hoursData || [] });
  };
  ...
  return (
    <>
      ...
      <div style={{ fontFamily: 'Poppins,sans-serif', maxWidth: 1200, margin: '0 auto' }}>
        <div className="ao-topbar" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, flexWrap: 'wrap', gap: 14 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            ...
            <select value={deptF} onChange={e => setDeptF(e.target.value)} style={{ background: C.surfaceHover, border: `1px solid ${C.border}`, borderRadius: 10, padding: '8px 14px', color: C.text, fontSize: 13, fontFamily: 'Poppins,sans-serif', outline: 'none', cursor: 'pointer' }}>
              {deptOptions.map(d => <option key={d} value={d} style={{ background: C.surface }}>{d === 'All' ? 'All Departments' : d}</option>)}
            </select>
          </div>
        </div>
        ...
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: 60, color: C.muted, fontSize: 14, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16 }}>No employees found</div>
        ) : (
