...
export default function LeaveManagementView(){
  ...
  const { createAllocation, isCreating: isAllocating } = useTimeOffAllocationMutations();
  ...
  // Employees for allocation dropdown
  ...
  allocations.forEach(a => {
    if (!allocByEmp[a.employeeName]) allocByEmp[a.employeeName] = { annual: 0, sick: 0, personal: 0, emergency: 0 };
    const key = a.leaveType.toLowerCase().includes('annual') ? 'annual'
      : a.leaveType.toLowerCase().includes('sick') ? 'sick'
      : a.leaveType.toLowerCase().includes('personal') ? 'personal'
      : a.leaveType.toLowerCase().includes('emergency') ? 'emergency' : 'annual';
    allocByEmp[a.employeeName][key] += a.totalDays;
  });
  ...
  const handleAllocate = async () => {
    ...
    try {
      ...
      setAllocErr(e?.response?.data?.message || e?.message || 'Failed to allocate leave.');
      ...
      <div style={{fontFamily:'Poppins,sans-serif',maxWidth:1100,margin:'0 auto'}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:12,marginBottom:32}}>
          ...
          {filtered.map((l,i)=>{
            ...
            return(
              <div key={l.id} className="lm-card" style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:'16px 20px',animationDelay:`${(i+3)*60}ms`,display:'flex',alignItems:'center',gap:16,flexWrap:'wrap'}}>
                ...
                <div className="lm-req-right" style={{display:'flex',alignItems:'center',gap:8,flexShrink:0}}>
                  ...
                  {isP&&<>
                    <button className="lm-btn" onClick={()=>approve(l.id)} disabled={isApproving} style={{background:C.teal,color:'#fff',opacity:isApproving?0.6:1}} onMouseEnter={e=>e.currentTarget.style.boxShadow=`0 4px 12px ${C.tealLight}`} onMouseLeave={e=>e.currentTarget.style.boxShadow='none'}>Approve</button>
        ...
        <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:16,padding:24,marginBottom:8}}>
          ...
          <div style={{display:'flex',gap:12,alignItems:'flex-end',flexWrap:'wrap',marginBottom:12}}>
            <div><label style={{fontSize:11,color:C.muted,display:'block',marginBottom:4}}>Employee</label>
              <select value={allocEmpId} onChange={e=>setAllocEmpId(e.target.value)} style={{background:C.surfaceHover,border:`1px solid ${C.border}`,borderRadius:10,padding:'9px 14px',color:C.text,fontSize:13,fontFamily:'Poppins,sans-serif',outline:'none',minWidth:180}}>
                <option value="" style={{background:C.surface}}>Select employee</option>
                {empList.map(e=><option key={e.id} value={e.id} style={{background:C.surface}}>{e.name}</option>)}
              </select>
            </div>
          ...
          {allocErr && <div style={{padding:'8px 14px',borderRadius:10,background:'rgba(239,68,68,0.1)',border:'1px solid rgba(239,68,68,0.25)',fontSize:12,color:'#EF4444',fontWeight:500,marginBottom:12}}>{allocErr}</div>}
          {allocMsg && <div style={{padding:'8px 14px',borderRadius:10,background:'rgba(20,184,166,0.1)',border:'1px solid rgba(20,184,166,0.25)',fontSize:12,color:C.teal,fontWeight:500,marginBottom:12}}>✓ {allocMsg}</div>}

          {Object.keys(allocByEmp).length > 0 && (
            <div style={{overflowX:'auto'}}>
              <table style={{width:'100%',borderCollapse:'collapse'}}>
                ...
                <tbody>
                  {Object.entries(allocByEmp).map(([emp, vals], i) => {
                    const total = vals.annual + vals.sick + vals.personal + vals.emergency;
                    return (
                      <tr key={emp} style={{background:i%2?C.surfaceHover:'transparent'}}>
                        <td style={{padding:'10px 12px',fontSize:13,fontWeight:500,color:C.text}}>{emp}</td>
                        <td style={{padding:'10px 12px',fontSize:13,color:C.teal}}>{vals.annual}</td>
                        <td style={{padding:'10px 12px',fontSize:13,color:C.danger}}>{vals.sick}</td>
                        <td style={{padding:'10px 12px',fontSize:13,color:C.accent}}>{vals.personal}</td>
                        <td style={{padding:'10px 12px',fontSize:13,color:C.warning}}>{vals.emergency}</td>
                        <td style={{padding:'10px 12px',fontSize:13,fontWeight:600,color:C.text}}>{total}</td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          {Object.keys(allocByEmp).length === 0 && <div style={{textAlign:'center',padding:20,color:C.muted,fontSize:13}}>No leave allocations yet</div>}
        </div>
      </div>
        ...
        <div className="lm-overlay" onClick={()=>setRejectModal(null)}>
          <div className="lm-modal" style={{maxWidth:400}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:'flex',gap:12,justifyContent:'flex-end'}}>
              ...
              <button onClick={()=>reject(rejectModal.id)} disabled={isRejecting} className="lm-btn" style={{background:C.danger,color:'#fff',padding:'10px 20px',opacity:isRejecting?0.6:1}}>{isRejecting?'Rejecting...':'Reject'}</button>
            </div>
          </div>
        </div>
      )}
