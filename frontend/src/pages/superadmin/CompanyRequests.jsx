...
export default function CompanyRequests() {
  ...
  // API Hooks
  const { data: statsData, loading: statsLoading } = useFetch('/company-requests/stats');
  const { data: reqData, loading: reqLoading, refetch: refetchReqs } = useFetch(`/company-requests?${queryParams.toString()}`);
  const { mutate: reviewReq, loading: reviewLoading } = useMutation('POST');
  ...
  const handleConfirmReview = async (id, action, note) => {
    try {
      await reviewReq(`/company-requests/${id}/review`, { action, note });
      setModalOpen(false);
      setSelectedReq(null);
