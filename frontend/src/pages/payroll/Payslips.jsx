              ...
              <div key={slip.id} className="ps-card" style={{ animationDelay: `${i * 50}ms` }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div>
                      <div style={{ fontSize: 15, fontWeight: 600, color: C.text }}>{slip.user?.name || 'Unknown'}</div>
                      <div style={{ fontSize: 11, color: C.muted, fontFamily: 'monospace' }}>{slip.user?.loginId || slip.user?.login_id || '—'}</div>
                    </div>
                  </div>
                  <span style={{ fontSize: 10, fontWeight: 600, padding: '4px 10px', borderRadius: 20, background: `${C.cyan}18`, color: C.cyan }}>
                    {new Date(slip.payrun?.periodStart || slip.payrun?.period_start || new Date()).toLocaleString('en-IN', { month: 'short', year: 'numeric' })}
                  </span>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16, background: C.bg, padding: 12, borderRadius: 10 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 14, fontWeight: 600, color: C.text }}>{slip.payableDays || slip.payable_days || 0}</div>
                  </div>
                  <div>
