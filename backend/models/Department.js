...
export async function listDepartments(db, { companyId, page = 1, limit = 10, search } = {}) {
    ...
    let base = 'SELECT id, company_id, name, created_at FROM departments WHERE company_id = $1';
...
export async function findDepartmentById(db, id) {
    const { rows } = await db.query('SELECT id, company_id, name FROM departments WHERE id = $1', [id]);
    return rows[0] || null;
}
