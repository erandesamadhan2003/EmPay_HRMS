...
export async function dashboardStats(req, res) {
	try {
		...
		const cacheKey = getDashboardStatsCacheKey(companyId);
...
export async function dashboardEmployerCost(req, res) {
	try {
		...
		// Use Redis cache with wrapper function
		const cacheKey = getDashboardEmployerCostCacheKey(companyId, year);
		const cached = await cacheWrapper(
			cacheKey,
			async () => {
				...
				return {
					year,
					view: "monthly",
					dataPoints: rows.map((r) => ({ label: r.label, amount: Number(r.amount) })),
				};
			},
			CACHE_EXPIRY.DASHBOARD,
		);

		return res.json(successResponse(cached, "Employer cost data fetched"));
	} catch (err) {
		console.error(err);
...
export async function dashboardEmployeeCount(req, res) {
	try {
		...
		// Use Redis cache with wrapper function
		const cacheKey = getDashboardEmployeeCountCacheKey(companyId, year);
		const cached = await cacheWrapper(
			cacheKey,
			async () => {
				const { rows } = await req.db.query(
				...
				return {
					year,
					view: "monthly",
					dataPoints: rows.map((r) => ({ label: r.label, count: r.count })),
				};
			},
			CACHE_EXPIRY.DASHBOARD,
		);

		return res.json(successResponse(cached, "Employee count data fetched"));
	} catch (err) {
		console.error(err);
...
export async function dashboardWarnings(req, res) {
	try {
		...
		// Use Redis cache with wrapper function
		const cacheKey = getDashboardWarningsCacheKey(companyId);
		const cached = await cacheWrapper(
			cacheKey,
			async () => {
				const q = `
		...
		return res.json(successResponse(cached, "Dashboard warnings fetched"));
	} catch (err) {
		console.error(err);
