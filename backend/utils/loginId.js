...
export async function generateLoginId(companyId, companyName, firstName, lastName, joiningDate) {
    ...
    // count existing users for company and year
    const q = `SELECT count(*) FROM users u JOIN employee_profiles e ON u.id = e.user_id WHERE e.company_id = $1 AND EXTRACT(YEAR FROM e.date_of_joining) = $2`;
    const r = await pool.query(q, [companyId, year]);
